...
from egppy.common.common import EGP_EPOCH
...
from egppy.gc_types.gc import GCABC, NULL_GC, NULL_PROBLEM, NULL_PROBLEM_SET, \
    PROPERTIES, GCBase, GCProtocol, encode_properties
...
class GGCMixin(EGCMixin, GCProtocol):
    ...
    def verify(self: GCProtocol) -> None:
        ...
        assert self['created'] <= datetime.now(UTC), \
        ...
        assert self['updated'] <= datetime.now(UTC), \
