...
from egppy.cg_graph.connections.connections_abc import ConnectionsABC
from egppy.cg_graph.connections.connections_class_factory import EMPTY_CONNECTIONS
from egppy.cg_graph.end_point.end_point import DstEndPointRef, EndPoint, SrcEndPointRef
from egppy.cg_graph.end_point.end_point_type import EndPointType, end_point_type
from egppy.cg_graph.end_point.types_def import types_db
from egppy.cg_graph.cg_graph_abc import GCGraphABC
from egppy.cg_graph.cg_validation import valid_dst_rows, valid_src_rows, GraphType
from egppy.cg_graph.interface import (
    EMPTY_INTERFACE,
...
from egppy.cg_graph.cg_key import (
    CPI,
    ROW_CLS_INDEXED,
    SOURCE_ROWS,
    SOURCE_ROW_SET,
...
class GCGraphMixin(CacheableObjMixin):
    ...
    def __init__(self, cg_graph: GCGraphDict | GCGraphABC) -> None:
        """Initialize the GC graph."""
        if isinstance(cg_graph, GCGraphABC) or not cg_graph:
            self._init_from_cg_graph(cg_graph)
        elif not isinstance(tuple(cg_graph.values())[0], list):
            self._init_from_dict(cg_graph)
        else:
            self._init_from_json(cg_graph)

        if _LOG_VERIFY:
            self.verify()
        if _LOG_CONSISTENCY:
            self.consistency()
            assert self.to_json() == cg_graph, "JSON GC Graph consistency failure."
        super().__init__()

    def _init_from_cg_graph(self, cg_graph: GCGraphABC | GCGraphDict) -> None:
        ...
        for key in ROW_CLS_INDEXED:
            self[key] = cg_graph.get(key, EMPTY_INTERFACE)
            self[key + "c"] = cg_graph.get(key + "c", EMPTY_CONNECTIONS)

    def _init_from_dict(self, cg_graph: GCGraphDict) -> None:
        """Initialize from a dictionary with keys and values like a GCGraphABC."""
        assert isinstance(self, GCGraphABC), "Invalid GC Graph type."
        rtype = self._TC.get_ref_iterable_type()
        for key in ROW_CLS_INDEXED:
            ckey = key + "c"
            if key not in cg_graph:
                self[key] = EMPTY_INTERFACE
                self[ckey] = EMPTY_CONNECTIONS
            elif ckey not in cg_graph:
                self[key] = cg_graph[key]
                self[ckey] = self._TC(rtype() for _ in cg_graph[key])

    def _init_from_json(self, cg_graph: GCGraphDict) -> None:
        ...
        for row, jeps in cg_graph.items():
            if row not in DESTINATION_ROW_SET_U:
                raise ValueError(
