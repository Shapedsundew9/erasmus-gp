...
Example:
    frozen_graph = FrozenGCGraph(c_graph_data)
    mutable_graph = MutableGCGraph(c_graph_data)
"""
...
from egppy.c_graph.c_graph_key import CGraphKey
from egppy.c_graph.c_graph_mixin import GCGraphDict, GCGraphMixin, key2parts
...
from egppy.c_graph.c_graph_key import ROW_CLS_INDEXED, DstRow, EPClsPostfix, SrcRow
...
class FrozenGCGraph(GCGraphMixin, GCGraphABC):
    ...
    def __init__(self, c_graph: GCGraphDict | GCGraphABC) -> None:
        ...
        self._graph_type = CGraphType.UNKNOWN

        # Call the mixin constructor & run the sanity checks
        super().__init__(c_graph)
        self._lock = True

    def __delitem__(self, key: CGraphKey) -> None:
    ...
    def _delete_interface(self, key: CGraphKey) -> None:
        """Delete an interface."""
        self._interfaces[key] = EMPTY_INTERFACE
        self._dirty_ics.add(key)

    def _delete_connections(self, key: CGraphKey) -> None:
        """Delete connections."""
        self._connections[key] = EMPTY_CONNECTIONS
        self._dirty_ics.add(key)

    def _delete_endpoint(self, key: CGraphKey) -> None:
        """Delete an endpoint."""
        ikey = key[0] + key[4]
        ckey = ikey + "c"
    ...
    def __getitem__(self, key: CGraphKey) -> Any:
    ...
    def __setitem__(self, key: CGraphKey, value: Any) -> None:
...
NULL_c_graph = FrozenGCGraph({})  # pylint: disable=abstract-class-instantiated
