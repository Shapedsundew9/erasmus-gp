...
from egppy.c_graph.cc_graph_class_factory import FrozenGCGraph, MutableGCGraph
...
def generate_valid_json_c_graphs() -> list[dict[str, list[list]]]:
    ...
    if not exists(filename):
        ...
        with open(filename, "w", encoding="ascii") as f:
            dump(jgc_graph_list, f, indent=4, sort_keys=True)
    else:
        # Read the JSON GC Graphs from a file
        with open(filename, "r", encoding="ascii") as f:
            jgc_graph_list = load(f)
    return jgc_graph_list
...
class GCGraphTestBase(unittest.TestCase):
    ...
    jgc_graph_list: list[dict[str, list[list]]] = generate_valid_json_c_graphs()
    ...
    @classmethod
    def get_gc_graph_cls(cls) -> type:
        """Get the Connections class."""
        return cls.gcgtype

    def setUp(self) -> None:
        self.gc_graph_type = self.get_gc_graph_cls()

    def test_valid_graphs(self) -> None:
        """Test the validity of the graphs."""
        if self.running_in_test_base_class():
            return
        for idx, jgcg in enumerate(self.jgc_graph_list):
            with self.subTest(idx=idx):
                gcg = self.gc_graph_type(jgcg)
                gcg.verify()
                gcg.consistency()
    ...
    def test_create_from_self(self):
        ...
        if self.running_in_test_base_class():
            return
        gcg1 = self.gc_graph_type(self.jgc_graph_list[0])
        gcg2 = self.gc_graph_type(gcg1)
    ...
    def test_is_stable(self):
        ...
        if self.running_in_test_base_class():
            return
        gcg = self.gc_graph_type(self.jgc_graph_list[0])
...
class MutableGCGraphTestBase(GCGraphTestBase):
    ...
    def test_equal_to_static(self) -> None:
        ...
        if self.running_in_test_base_class():
            return
        sgcg = FrozenGCGraph(self.jgc_graph_list[0])
        dgcg = self.gc_graph_type(sgcg)
    ...
    def test_is_stable2(self):
        ...
        if self.running_in_test_base_class():
            return
        for jgcg in self.jgc_graph_list:
            gcg = self.destablize_graph(self.gc_graph_type(jgcg))
