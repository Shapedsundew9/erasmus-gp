...
class Validator:
    ...
    def _is_bool(self, attr: str, value: Any) -> bool:
        ...
        if not result:
            _logger.log(TRACE, "%s must be a bool but is %s", attr, type(value))
        return result
    ...
    def _is_bytes(self, attr: str, value: Any) -> bool:
        ...
        if not result:
            _logger.log(TRACE, "%s must be bytes but is %s", attr, type(value))
        return result
    ...
    def _is_callable(self, attr: str, value: Any) -> bool:
        ...
        if not result:
            _logger.log(TRACE, "%s must be callable but is %s", attr, type(value))
        return result
    ...
    def _is_datetime(self, attr: str, value: Any) -> bool:
        ...
        if not result:
            _logger.log(TRACE, "%s must be a datetime but is %s", attr, type(value))
        return result
    ...
    def _is_dict(self, attr: str, value: Any) -> bool:
        ...
        if not result:
            _logger.log(TRACE, "%s must be a dict but is %s", attr, type(value))
        return result
    ...
    def _is_hash8(self, attr: str, value: Any, _assert: bool = True) -> bool:
        ...
        if not result:
            _logger.log(TRACE, "%s must be a hash8 but is %s", attr, value)
        return result
    ...
    def _is_historical_datetime(self, attr: str, value: Any) -> bool:
        ...
        if not result:
            _logger.log(
                TRACE, "%s must be a post-EGP epoch historical datetime but is %s", attr, value
            )
        return result
    ...
    def _is_hostname(self, attr: str, value: str) -> bool:
        """Validate a hostname."""
        if len(value) > 255:
            _logger.log(TRACE, "%s must be a valid hostname: %s is >255 chars.", attr, value)
            return False
        ...
        if not result:
            _logger.log(TRACE, "%s must be a valid hostname: %s is not valid.", attr, value)
        return result
    ...
    def _is_instance(self, attr: str, value: Any, cls: type | tuple[type, ...]) -> bool:
        ...
        if not result:
            _logger.log(TRACE, "%s must be an instance of %s but is %s", attr, cls, type(value))
        return result
    ...
    def _is_int(self, attr: str, value: Any) -> bool:
        ...
        if not result:
            _logger.log(TRACE, "%s must be an int but is %s", attr, type(value))
        return result
    ...
    def _is_ip(self, attr: str, value: str) -> bool:
        ...
        try:
            ...
            _logger.log(TRACE, "%s must be a valid IP address: %s is not valid.", attr, value)
        return False
    ...
    def _is_list(self, attr: str, value: Any) -> bool:
        ...
        if not result:
            _logger.log(TRACE, "%s must be a list but is %s", attr, type(value))
        return result
    ...
    def _is_not_none(self, attr: str, value: Any) -> bool:
        ...
        if not result:
            _logger.log(TRACE, "%s must not be None", attr)
        return result
    ...
    def _is_one_of(self, attr: str, value: Any, values: Iterable[Any]) -> bool:
        ...
        if not result:
            _logger.log(TRACE, "%s must be one of %s but is %s", attr, values, value)
        return result
    ...
    def _is_printable_string(self, attr: str, value: str) -> bool:
        ...
        if not result:
            _logger.log(TRACE, "%s must be a printable string but is not.", attr)
        return result
    ...
    def _is_regex(self, attr: str, value: str, pattern: Pattern) -> bool:
        ...
        if not result:
            _logger.log(TRACE, "%s must match the pattern %s but does not", attr, pattern)
        return bool(result)
    ...
    def _is_sequence(self, attr: str, value: Any) -> bool:
        ...
        if not result:
            _logger.log(TRACE, "%s must be a sequence but is %s", attr, type(value))
        return result
    ...
    def _is_sha256(self, attr: str, value: Any) -> bool:
        ...
        if not result:
            _logger.log(TRACE, "%s must be a SHA256 hash but is %s", attr, value)
        return result
    ...
    def _is_string(self, attr: str, value: Any) -> bool:
        ...
        if not result:
            _logger.log(TRACE, "%s must be a string but is %s", attr, type(value))
        return result
    ...
    def _is_tuple(self, attr: str, value: Any) -> bool:
        ...
        if not result:
            _logger.log(TRACE, "%s must be a tuple but is %s", attr, type(value))
        return result
    ...
    def _is_url(self, attr: str, value: str) -> bool:
        ...
        if presult.scheme != "https" or not presult.netloc:
            _logger.log(TRACE, "%s must be a valid URL: %s is not valid.", attr, value)
            return False
    ...
    def _is_uuid(self, attr: str, value: Any) -> bool:
        ...
        if not result:
            _logger.log(TRACE, "%s must be a UUID but is %s", attr, type(value))
        return result
