...
| Harmony | Combine two GCs into a synchronized structure, merging or aligning their interfaces. |
| Perfect Stack | Stack one GC on top of another such that all destination endpoints are perfectly satisfied without stabilization. |
| Stack | Stack one GC on top of another, potentially leaving unconnected endpoints that require subsequent stabilization. |
| Insert | Insert a GC into the target GC's graph, replacing an existing connection or wrapping an existing node. |
| Swap | Swap two sub-GCs or specific codons within the target GC graph. |
| Delete | Remove a sub-GC or codon from the target graph, rerouting or exposing its connections for stabilization. |
| Split | Introduce an if-then-else conditional branching structure into the GC graph. |
| Bypass | Introduce an if-then conditional bypass around a specific sub-GC. |
| Iterate | Wrap a sub-GC within a for-loop iterative structure. |
