...
register_token_code(
    "I05005", "Database {dbname} does not yet exist. Waiting {backoff:.2}s to retry."
)
...
register_token_code(
    "E05002",
    "Existing database table {table} columns do not match configuration."
    " Column {column} PRIMARY KEY constraint is inconsistent.",
)
register_token_code(
    "E05003",
    "Existing database table {table} columns do not match configuration."
    " Column {column} UNIQUE constraint is inconsistent.",
)
register_token_code(
    "E05004",
    "Existing database table {table} columns do not match configuration."
    " Column {column} NOT NULL constraint is inconsistent.",
)
register_token_code(
    "E05005",
...
register_token_code(
    "E05006", "Recursive select on table {table} requires ptr_map to be configured."
)
register_token_code(
    "E05007",
...
_TABLE_RECURSIVE_SELECT = sql.SQL(
    "WITH RECURSIVE rq AS (SELECT {0} FROM {1} {2} UNION {5}SELECT {3} FROM {1} t INNER JOIN"
    " rq r ON {4}) SELECT * FROM rq"
)
...
class RawTable:
    ...
    def _table_definition(self) -> set[str]:
        ...
        schema = {c: ColumnSchema(db_type=d.upper(), nullable=n == "YES") for c, d, n in results}
        constraints = self._db_transaction(
            _TABLE_GET_PRIMARY_KEY_SQL.format(sql.Literal(self.config["table"]))
        )
        for column, constraint in constraints:
            schema[column]["primary_key"] = (constraint == "PRIMARY KEY") or schema[column].get(
                "primary_key", False
            )
            schema[column]["unique"] = (constraint == "UNIQUE") or schema[column].get(
                "primary_key", False
            )
    ...
    def recursive_select(
        ...
        Recursive select will return all the rows defined by the query_str plus the union
        of any rows they point to and the rows those rows point to...recursively until no
        references are left (or are not in the table).
        ...
        ctype: One of 'tuple', 'namedtuple', 'dict'
        dedupe: Duplicate entries are removed from the result when True.

        Returns
        -------
        A psycopg2 cursor of a type defined by ctype of the values specified by columns for
        the specified recursive query_str and pointer map.
            'tuple': TupleCursor
            'namedtuple': NamedTupleCursor
            'dict': DictCursor
        """
    ...
    def upsert(
        ...
        value. If columns = ['column1'] and values = [(10,)], literals = {'one': 1} and the table
        name is 'test_table' the example update_str would result in the following SQL:
        ...
        returning: The columns to be returned on update. If None or empty no columns will
        be returned.
        ...
        Returns
        -------
        A psycopg2 cursor of a type defined by ctype of the values specified by returning for
        each updated row:
            'tuple': TupleCursor
            'namedtuple': NamedTupleCursor
            'dict': DictCursor
        ...
        format_dict.update(
            {"EXCLUDED." + k: sql.SQL("EXCLUDED.") + sql.Identifier(k) for k in columns}
        )
        update_sql = sql.SQL(update_str).format(**format_dict)
        if returning:
            update_sql += _TABLE_RETURNING_SQL + sql.SQL(",").join(
