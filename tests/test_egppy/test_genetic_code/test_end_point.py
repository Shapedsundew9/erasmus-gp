...
class TestEndPoint(unittest.TestCase):
    ...
    def test_to_json(self) -> None:
        ...
        ep_dst_no_refs = EndPoint(DstRow.A, 0, EndPointClass.DST, "int")
        with self.assertRaises(ValueError):
            ep_dst_no_refs.to_json(json_c_graph=True)

    def test_init_from_tuple(self) -> None:
        """Test initialization from a 5-tuple."""
        tuple_data = (SrcRow.I, 0, EndPointClass.SRC, types_def_store["int"], [["A", 1]])
        ...
        # Test with None refs
        tuple_data_no_refs = (SrcRow.I, 0, EndPointClass.SRC, types_def_store["int"], None)
    ...
    def test_init_from_endpoint(self) -> None:
        ...
        original = EndPoint(SrcRow.I, 0, EndPointClass.SRC, "int", [["A", 1]])
    ...
    def test_init_with_types_def_instance(self) -> None:
        ...
        ep = EndPoint(SrcRow.I, 0, EndPointClass.SRC, typ)
    ...
    def test_init_invalid_arguments(self) -> None:
        ...
        with self.assertRaises(TypeError):
            EndPoint(("I", 0, EndPointClass.SRC))
        ...
        with self.assertRaises(TypeError):
            EndPoint("I", 0, EndPointClass.SRC, "int", [], "extra")

    def test_str_representation(self) -> None:
        """Test string representation of endpoints."""
        ep = EndPoint(SrcRow.I, 0, EndPointClass.SRC, "int", [["A", 1]])
        str_repr = str(ep)
    ...
    def test_comparison_with_non_endpoint(self) -> None:
        ...
        ep = EndPoint(SrcRow.I, 0, EndPointClass.SRC, "int")
...
class TestEndPointEdgeCases(unittest.TestCase):
    ...
    def test_hash_consistency_with_equality(self) -> None:
        ...
        ep1 = EndPoint(SrcRow.I, 0, EndPointClass.SRC, "int", [["A", 0]])
        ep2 = EndPoint(SrcRow.I, 0, EndPointClass.SRC, "int", [["A", 0]])
        self.assertEqual(ep1, ep2)
    ...
    def test_equality_different_refs_order(self) -> None:
        ...
        ep1 = EndPoint(SrcRow.I, 0, EndPointClass.SRC, "int", [["A", 0], ["A", 1]])
        ep2 = EndPoint(SrcRow.I, 0, EndPointClass.SRC, "int", [["A", 1], ["A", 0]])
        self.assertNotEqual(ep1, ep2)
    ...
    def test_comparison_same_idx(self) -> None:
        ...
        ep1 = EndPoint(SrcRow.I, 5, EndPointClass.SRC, "int")
        ep2 = EndPoint(SrcRow.I, 5, EndPointClass.SRC, "float")
    ...
    def test_verify_all_valid_rows(self) -> None:
        ...
        for row in SrcRow:
            ...
            ep = EndPoint(row, idx, EndPointClass.SRC, "int")
        ...
        for row in DstRow:
            ...
            idx = 0 if row in SINGLE_ONLY_ROWS else 5
            ep = EndPoint(row, idx, EndPointClass.DST, "int")
            ep.verify()  # Should not raise

    def test_to_json_with_multiple_refs(self) -> None:
        """Test to_json() with multiple references."""
        ep = EndPoint(SrcRow.I, 0, EndPointClass.SRC, "int", [["A", 0], ["A", 1], ["B", 2]])
        json_obj = ep.to_json()
    ...
    def test_init_refs_deep_copy(self) -> None:
        ...
        ep = EndPoint(SrcRow.I, 0, EndPointClass.SRC, "int", original_refs)
    ...
    def test_boundary_index_values(self) -> None:
        ...
        # Index 0 (minimum)
        ep0 = EndPoint(SrcRow.I, 0, EndPointClass.SRC, "int")
        ep0.verify()

        # Index 255 (maximum)
        ep255 = EndPoint(SrcRow.I, 255, EndPointClass.SRC, "int")
        ep255.verify()
    ...
    def test_different_row_same_idx_comparison(self) -> None:
        ...
        ep_src = EndPoint(SrcRow.I, 5, EndPointClass.SRC, "int")
        ep_dst = EndPoint(DstRow.A, 5, EndPointClass.DST, "int")
...
class TestEndPointVerify(unittest.TestCase):
    ...
    def test_verify_index_range(self) -> None:
        ...
        # Negative index
        ep = EndPoint(SrcRow.I, -1, EndPointClass.SRC, "int")
        with self.assertRaises(ValueError) as cm:
            ep.verify()
        self.assertIn("must be between 0 and 255", str(cm.exception))

        # Index too large
        ep = EndPoint(SrcRow.I, 256, EndPointClass.SRC, "int")
        with self.assertRaises(ValueError) as cm:
            ep.verify()
        self.assertIn("must be between 0 and 255", str(cm.exception))
    ...
    def test_verify_row_u_invalid(self) -> None:
        ...
        ep = EndPoint(DstRow.U, 0, EndPointClass.DST, "int")
        with self.assertRaises(ValueError) as cm:
            ep.verify()
    ...
    def test_verify_dst_row_class_compatibility(self) -> None:
        ...
        ep = EndPoint(SrcRow.I, 0, EndPointClass.DST, "int")
        with self.assertRaises(ValueError) as cm:
            ep.verify()
    ...
    def test_verify_src_row_class_compatibility(self) -> None:
        ...
        ep = EndPoint(DstRow.O, 0, EndPointClass.SRC, "int")
        with self.assertRaises(ValueError) as cm:
            ep.verify()
    ...
    def test_verify_single_only_row_constraint(self) -> None:
        ...
        ep = EndPoint(DstRow.F, 1, EndPointClass.DST, "int")
        with self.assertRaises(ValueError) as cm:
            ep.verify()
        self.assertIn("can only have a single endpoint", str(cm.exception))

        # W row can only have index 0
        ep = EndPoint(DstRow.W, 5, EndPointClass.DST, "int")
        with self.assertRaises(ValueError) as cm:
            ep.verify()
        self.assertIn("can only have a single endpoint", str(cm.exception))

        # L row (source) can only have index 0
        ep = EndPoint(SrcRow.L, 2, EndPointClass.SRC, "int")
        with self.assertRaises(ValueError) as cm:
            ep.verify()
        self.assertIn("can only have a single endpoint", str(cm.exception))
    ...
    def test_verify_src_must_reference_dst_rows(self) -> None:
        ...
        ep = EndPoint(SrcRow.I, 0, EndPointClass.SRC, "int")
        ep.refs = [["I", 0]]  # I is a source-only row
        with self.assertRaises(ValueError) as cm:
            ep.verify()
    ...
    def test_verify_valid_endpoints(self) -> None:
        ...
        # Valid SRC endpoint
        ep_src = EndPoint(SrcRow.I, 0, EndPointClass.SRC, "int", [["A", 0], ["A", 1]])
        ep_src.verify()  # Should not raise

        # Valid DST endpoint
        ep_dst = EndPoint(DstRow.A, 5, EndPointClass.DST, "float", [["I", 0]])
        ep_dst.verify()  # Should not raise

        # Valid endpoint with no refs
        ep_no_refs = EndPoint(SrcRow.B, 10, EndPointClass.SRC, "int")
